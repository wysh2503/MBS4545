// Three ultrasonic sensors - three publish nodes
// Kalman filter
// Ping
//
#include <NewPing.h>
...
#define SENSOR_NUM 3          //The number of HC-SR04 
#define MAX_DISTANCE 200     //Max distance to detect obstacles [in cm]
...
int LOOPING = 40; //Loop every 40 milliseconds
...
void sensorCycle() {
  for (uint8_t i = 0; i < SENSOR_NUM; i++) {
    if (millis() >= pingTimer[i]) {
      ...
      currentSensor = i;
      cm[currentSensor] = 0;
      sensor[currentSensor].ping_timer(echoCheck);
    }
  }
}
...
void echoCheck() {
  if (sensor[currentSensor].check_timer())
    cm[currentSensor] = sensor[currentSensor].ping_result / US_ROUNDTRIP_CM;
}
...
{
  range_name.radiation_type = sensor_msgs::Range::ULTRASONIC;
...
ros::Publisher pub_range_left("/ultrasonic_left", &range_left);
ros::Publisher pub_range_center("/ultrasonic_center", &range_center);
ros::Publisher pub_range_right("/ultrasonic_right", &range_right);
...
void setup() {
  ...
  sensor_msg_init(range_left, "/ultrasonic_left");
  sensor_msg_init(range_center, "/ultrasonic_center");
  sensor_msg_init(range_right, "/ultrasonic_right");
}
